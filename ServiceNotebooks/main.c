...
eTrabajo trabajos[TAMTRABAJO] = {{1,100,20001,{20,10,2020},0},{2,100,20000,{20,10,2020},0},{3,103,20003,{1,5,2019},0},{4,102,20002,{19,1,2020},0}};
char salir='n';
int proxId=100,indice,idTrabajo=1;

inicializarNotebook(notebooks,TAMNOTE);
hardcodeNotebook(notebooks,TAMHARD);
inicializarTrabajo(trabajos,TAMTRABAJO,4);
idTrabajo=+HARDTRA;
proxId=+TAMHARD;
...
switch(menuNotebook())
{
	...
	case 'd':
        ...
        mostrarNotebooks(notebooks,TAMNOTE,marcas,TAMM,tipos,TAMTIPO);
        }else
            printf("Debe generar por lo menos un alta.\n\n");
            system("pause");
		break;
	...
	case 'h':
        ...
        printf("*****Alta de Trabajos*****\n");
         if(hayRegistros(notebooks,TAMNOTE))
        {
            if(altaTrabajo(trabajos,TAMTRABAJO,notebooks,TAMNOTE,servicios,TAMSERV,marcas,TAMM,tipos,TAMTIPO,idTrabajo))
            {
