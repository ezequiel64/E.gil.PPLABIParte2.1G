...
int altaNotebook(eNotebook list[],int tam,eTipo tipo[],int tamTipo,eMarca marca[],int tamMarca,eCliente cliente[],int tamCliente,int id,int indice)
{
   int rta = 0;
    eNotebook auxNote;
    auxNote.isEmpty=0;
...
int bajaNotebook(eNotebook list[],int tamNote,eTipo tipo[],int tamTipo,eMarca marca[],int tamMar,eCliente cliente[], int tamCliente)
{
    ...
    case 1:
     if(getInt(&id,1,100,150,"Ingrese el id de la notebook que desea dar de baja: ","Error. Ingrese el id de la notebook que desea dar de baja: "))
            ...
            if (-1<(indice=buscarNotebook(list,tamNote,id)))
            {
                ...
                mostrarNotebook(list[indice],marca,tamMar,tipo,tamTipo,cliente,tamCliente);
...
void mostrarNotebook(eNotebook list,eMarca marca[],int tamMarca,eTipo tipo[],int tamTipo,eCliente cliente[],int tamCliente)
{
    char descripcionMarca[20],descripcionTipo[20],nombreCliente[20];
    ...
    cargarNombreCliente(nombreCliente,list.idCliente,cliente,tamCliente);
    printf("%3d       %6s      %10s   %10s     %.2f     %10s\n",list.id,descripcionMarca,descripcionTipo,list.modelo,list.precio,nombreCliente);

}

void mostrarNotebooks(eNotebook list[] ,int tam,eMarca marca[],int tamMarca,eTipo tipo[],int tamTipo,eCliente cliente[],int tamCliente)
{
...
for(int i=0;i<tam;i++)
{
    if(!list[i].isEmpty)
    {
        if(!flag)
        {
        printf("Id         Marca          Tipo          Modelo       Precio       Nombre de Cliente\n");
        flag=1;
        }
    mostrarNotebook(list[i],marca,tamMarca,tipo,tamTipo,cliente,tamCliente);
    }
